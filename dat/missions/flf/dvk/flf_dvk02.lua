...
"Thank you for your service, {player}. It sounds like we may have been clear of any real danger already if it's true that they're looking for so-called aliens in the Nebula, but that little bit of intimidation and offer of cooperation should solidify the safety of our position. We may very soon start seeïng reduced hostility from pirates as a result, which helps us further."]])

pay_text = _([[Benito hands you payment on a credit chip before continuing. "I must admit I'm also curious about their whole 'aliens' ideä. We don't tend to really explore the Nebula since we only use it for cover, but I have indeed heard the ghost stories. Perhaps we should do our own investigation. I'm not sure if I believe them, but if there really are aliens, making allies out of them could be very helpful to our cause. Of course, we can't put too many resources into that as the so-called ghosts could maybe just be some pre-Incident artifacts that somehow survived beïng turned to dust by the nebula, but it wouldn't hurt to do some cursory investigation and trade information with the pirates.
