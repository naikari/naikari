...
Source0: http://naikari.googlecode.com/files/%{name}-%{version}.tar.bz2
Source1: naikari.png
Source2: http://naikari.googlecode.com/files/ndata-%{version}
...
%build
%configure2_5x --with-ndata-path=/usr/share/naikari/ndata-%{version}
...
rm -rf %{buildroot}
%makeinstall_std
install -m644 %{SOURCE1} -D %{buildroot}%{_iconsdir}/naikari32.png
install -m644 %{SOURCE2} -D %{buildroot}%{_datadir}/naikari/ndata-%{version}
...
%attr(755,root,root) %{_bindir}/naikari

%{_mandir}/man6/naikari.6.*
%{_datadir}/applications/naikari.desktop
%{_iconsdir}/naikari32.png

%files -n %{dataname}
%defattr(-,root,root)
%{_datadir}/naikari/ndata-%{version}
