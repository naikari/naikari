...
if [[ $NIGHTLY == true ]]; then
    if [[ $BETA == true ]]; then 
        makensis -DVERSION=$BASEVER.0 -DVERSION_SUFFIX=-beta.$BETAVER-$BUILD_DATE -DARCH=$ARCH extras/windows/installer/naikari.nsi
    elif [[ $BETA == false ]]; then 
        makensis -DVERSION=$VERSION -DVERSION_SUFFIX=-$BUILD_DATE -DARCH=$ARCH extras/windows/installer/naikari.nsi
    else
...
mv extras/windows/installer/naev-$VERSION-$BUILD_DATE-win$ARCH.exe dist/release/naev-win$ARCH.exe
    ...
    if [[ $BETA == true ]]; then 
        makensis -DVERSION=$BASEVER.0 -DVERSION_SUFFIX=-beta.$BETAVER -DARCH=$ARCH extras/windows/installer/naikari.nsi
    elif [[ $BETA == false ]]; then 
        makensis -DVERSION=$VERSION -DVERSION_SUFFIX= -DARCH=$ARCH extras/windows/installer/naikari.nsi
    else
        echo "Something went wrong determining if this is a beta or not."
    fi

# Move installer to distribution directory
mv extras/windows/installer/naikari-$VERSION-win$ARCH.exe dist/release/naikari-win$ARCH.exe
else
...
zip ../../../../dist/release/naikari-win$ARCH.zip *.dll *.exe
cd ../../../../
