      ...
      int selected, Outfit *o, Pilot *p, CstSlotWidget *wgt )
{
   ...
   for (i=0; i<array_size(lst); i++) {
      ...
      if (lst[i].outfit != NULL) {
         ...
         toolkit_drawOutlineThick( x, y, w, h, -.05*w, .20*w, (irrelevant ? &ic : &cBlack), 0 );
         if (!ok) {
            toolkit_drawTriangle( x+.05*w, y+.25*h, x+.75*w, y+.95*h, x+.25*w, y+.075*h, (irrelevant ? &ic : &cBlack) );
            toolkit_drawTriangle( x+.95*w, y+.75*h, x+.75*w, y+.95*h, x+.25*w, y+.075*h, (irrelevant ? &ic : &cBlack) );
            toolkit_drawTriangle( x+.10*w, y+.20*h, x+.80*w, y+.90*h, x+.20*w, y+.10*h, (irrelevant ? &ic : &cRed) );
            toolkit_drawTriangle( x+.90*w, y+.80*h, x+.80*w, y+.90*h, x+.20*w, y+.10*h, (irrelevant ? &ic :&cRed) );
         }
         toolkit_drawOutlineThick( x, y, w, h, -.10*w, .10*w, (irrelevant ? &ic : (ok ? &cGreen : &cRed)), 0 );
      }
