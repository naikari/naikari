...
usage() {
    ...
    echo "Pass in -t <Github PAT> -r <release type> e.g. (nightly, prerelease, release) -g <github repo name> defaults to: (naikari/naikari) -b <ref/branch name> defaults to: (main)"
    exit 1
}
...
case $RELEASE_TYPE in
  "nightly")
    WORKFLOW_FILE_NAME="naikari_$RELEASE_TYPE.yml"
    ;;
  "prerelease")
    WORKFLOW_FILE_NAME="naikari_$RELEASE_TYPE.yml"
    ;;
