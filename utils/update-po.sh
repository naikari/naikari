...
if [[ ! -f "$ROOT/naikari.6" ]]; then
   echo "Please run from Naikari root directory, or run with update-po.sh [source_root]" >&2
   exit -1
fi
...
"$ROOT/po/credits_pot.py" dat/intro dat/AUTHORS artwork/gfx/loading/*.txt > "$ROOT/po/credits.pot"
